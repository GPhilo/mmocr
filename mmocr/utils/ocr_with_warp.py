...
class MMOCR:
    ...
    def readtext(self,
        ...
        if self.detect_model and self.recog_model:
            det_recog_result = self.det_recog_kie_inference(
                self.detect_model, self.recog_model, kie_model=self.kie_model, use_warping=use_warping)
    ...
    def det_recog_kie_inference(self, det_model, recog_model, kie_model=None, use_warping=False):
        end2end_res = []
        ...
        for filename, arr, bboxes, out_file in zip(self.args.filenames,
            ...
            for bbox in bboxes:
                ...
                if len(bbox) > 9:
                    if use_warping:
                        # We got a polygon back, fit a rotated rect and use that for warping
                        box = boxPoints(minAreaRect(np.asarray(bbox[:-1]).reshape([-1,2]).astype(np.float32)))
                        # TODO?: Add all 4 possible rotations of the points and adapt recog inference below to only keep the result with the highest overall score
                        #       instead of relying on internal sorting of points? (pts are sorted from TL clockwise before warping)
                    else:
    ...
    def _args_processing(self, args):
        ...
        if isinstance(args.img, str):
            ...
            if img_path.is_dir():
                img_list = [str(x) for x in img_path.glob('*') if x.is_file()]
            else:
